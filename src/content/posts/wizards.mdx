---
title: Wizards in Ruby on Rails
date: 2024-07-10
description: How we implemented multi-step forms (wizards) in Ruby on Rails, with insights on best practices, common gems, and a reusable, session-based pattern. Perfect for GOV.UK-style services and beyond.
---

<Callout type="note">
This post was adapted from an [architectural decision record][adr] I wrote while working at the UK Department for Education.

My team needed to implement various multi-step forms in our Ruby on Rails application. This was the output of an investigation I carried out.
</Callout>

Digital services that follow the [GOV.UK Service Standard][1] tend to make heavy use of multi-step forms. This approach is [considered best practice][2] because it breaks down complex tasks into smaller, easier to follow steps – for example, when [publishing a school placement][3], or [claiming funding for mentor training][4].
...
By their nature, wizards don't cleanly fit into that model. Users submit several forms, split across several pages, and the resultant thing can only be created once the user has completed every step of the wizard. If you're not careful, you can easily end up fighting the framework as you try to shoehorn wizard behaviour into a CRUD controller.

I've therefore found it helpful to abstract common wizard behaviour into a set of classes and conventions that make it easier to build multi-step forms in Rails.

## Options for building wizards in Rails

One common approach for implementing multi-step forms is to build them as 'single page apps' using JavaScript, then submit everything to the 'create' endpoint as one single payload. But this isn't always an option, for example if the form needs to [work without JavaScript][7].
...
### 2. Wizards won't always create things
...
We defined the following boundaries for controllers, wizards and steps:
...
## A pattern for implementing wizards

Given all of the above, we came up with a pattern for implementing wizards in our application. It loosely follows the structure of the DfE Wizard gem, where wizards and steps are standalone classes. But it stores its state in the session, and the wizard itself is responsible for the user's journey through the form.

### Can I use it in my own Ruby on Rails project?

Of course! One of the great things about digital services created by the UK government is that [the source code must be public][14].

For a concrete implementation, see this pull request which establishes the pattern and implements an "Add placement" wizard:  
[DFE-Digital/itt-mentor-services#810][15]

I also [documented the pattern][16] to help developers in my team create new wizards in future.
...
[adr]: https://github.com/DFE-Digital/itt-mentor-services/blob/c22adb931928994fae2256279bc9399d74bac248/adr/00009-wizards.md
...
[14]: https://www.gov.uk/service-manual/technology/making-source-code-open-and-reusable
[15]: https://github.com/DFE-Digital/itt-mentor-services/pull/810
[16]: https://github.com/DFE-Digital/itt-mentor-services/blob/a803dc98545c17cdac3c3bda9c8aea88be721986/docs/conventions/wizards.md
